...
## Layer 1.5 — cart + checkout write path (SHIPPED 2026-06-29, branch feat/krafta-studio)

**Status:** built + e2e-verified on the dev Supabase branch, commits `b035025` (#14a cart_sessions + createCart/getCart), `3caf0d5` (#14b setLines), `2bbd148` (#14c pricing + checkout + getOrder). Whole-app `tsc` clean. The migration `20260629130000_commerce_cart_sessions.sql` is applied to dev and staged for prod. Verified flow against the real Teplo Fest shop: createCart → setLines (engine re-prices; a forged body price was ignored) → pricing → checkout (pickup, cash) → getOrder; cross-shop / wrong-token requests return 404. Notable build details vs the original design: `clientForCartToken` attaches the anon user's access token via a **global `Authorization` header** (role=`authenticated`, where the commerce-table grants live) rather than relying on `refreshSession`'s in-memory session, which supabase-js does not reliably bind to PostgREST/RPC calls; `commerce.cart_sessions` is typed via casts until `supabase gen types` is rerun (same posture as `commerce.api_keys`). Follow-ups: rate-limit `POST /carts` (each call mints an anon user — natural cap is Supabase's anon-signin limit); a `GET /orders` list (secret-key only) is not built (only the cartToken-scoped per-order read).
