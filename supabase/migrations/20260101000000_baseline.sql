...
--
-- Why: this baseline is a pg_dump of prod, so it captures the *cumulative
-- effect* of every migration that ever ran. Replaying the 21 incremental
-- migrations on top would re-create things (triggers, functions, etc.)
-- that don't all support IF NOT EXISTS — Postgres has no
-- `CREATE TRIGGER IF NOT EXISTS`. Marking them as applied here lets the
-- migration runner skip them, while NEW migrations (any added after this
-- baseline lands) still apply normally.
--
-- Also some migrations were deleted from `supabase/migrations/` over time
-- but their effects remain in prod (and therefore in this baseline). The
-- versions below are what the runner currently sees locally; missing files
-- aren't the runner's concern (it only iterates what exists on disk).
-- =========================================================================
