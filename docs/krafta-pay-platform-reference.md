...
## Subscription renewals (current vs pending)

### Implemented in domain logic

- Renewal charge logic and retry policy primitives exist in payments core
- Off-session charging uses provider token (`bindingId`)

### Operationally pending / hardening in progress

- Production renewal scheduler/cron wrapper endpoint for Vercel
- Monitoring and operator-friendly renewal run visibility
- Full dunning operations UI
...
- `webhook_route`
- `webhook`
- `cron` (reserved/expected future usage)
...
## `type=cron` (planned/expected)

Reserved for renewal scheduler execution logs. Recommended future events:
...
- Renewal cron wrapper endpoint for easy Vercel Cron integration
